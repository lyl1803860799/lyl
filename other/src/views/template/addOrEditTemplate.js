...
import {templateDetail,addTemplate,editTemplate,getUnNamedTemplateCount} from '@/api/template'
...
export default {
  ...
  async mounted() {
    ...
    this.templateParentId = this.$route.query?.templateParentId || null;//分组id回显
    if(!this.pageId){
      this.getUnNamedTemplate();
    }
    window.localStorage.removeItem("rightWidth");
    // 初始化list
  ...
  methods: {
      ...
      try {
        ...
        if(this.pageId){
          editTemplate({
            ...
            templateName: this.pageInfo.dataViewName,
            materialsConfig: JSON.stringify(listObj),
            templateConfig: JSON.stringify(this.pageOption),
            templateSnapshot:snapshot,
            templateSort:null,
            groupOwnerId: this.pageOption.groupOwnerId,
            templateLimitType:-1,
            templateSize: width+'*'+height
          }).then(res=>{
            this.loading = false
            if(res.success){
              this.templateParentId = this.pageOption.groupOwnerId;
              this.viewData = true
              ...
              if(isLeave){
                this.$router.push({ name: '/index/templateManage',params:{treeUniqId:this.treeUniqId} })
              }
            }else{
    ...
    handlePageOptionChange(val) {
      val.defineSize=val.screenSize.width+"*"+val.screenSize.height
    ...
    async getPageInfo() {
      ...
      try {
        const { data } = await templateDetail({ templateId: this.pageId,groupType:this.pageGropType })
        this.loading = false
        ...
        pageOption.groupOwnerId=this.templateParentId;
    ...
    getUnNamedTemplate(){
      getUnNamedTemplateCount().then(res=>{
        let num = res.data==0?'':res.data
        this.pageInfo.dataViewName = '未命名模板' + num;
      });
    },
  },
