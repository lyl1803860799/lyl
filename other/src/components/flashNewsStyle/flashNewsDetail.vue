<template>
  <a-form :form="form" :colon="false" labelAlign="left">
    ...
    <a-form-item v-if="type!='time'" label="背景颜色" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18 }">
      <color-input :show-input="true" :elId="'newsBgColor'+type" :placement="'top'"  v-decorator="['backgroundColor', { initialValue: formData.backgroundColor }]"></color-input>
    </a-form-item>
    <a-form-item v-if="type==='flash'" label="行间距" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18 }">
      <a-input-number
        :min="12"
        :step="1"
        :precision="0"
    ...
    <a-form-item  v-if="type==='flash'" label="字间距" :labelCol="{ span: 5 }" :wrapperCol="{ span: 18 }">
      <a-input-number
        :min="0"
        :step="1"
        :precision="0"
