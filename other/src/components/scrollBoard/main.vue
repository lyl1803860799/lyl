    ...
    >
      <div
        ...
        :style="
          ...
          background-color: ${mergedConfig[row.rowIndex % 2 === 0 ? 'oddRowBGC' : 'evenRowBGC']};
        `
        "
      >
        <div
...
import { deepMerge, deepClone } from '@/utils/tool'
...
export default {
  ...
  methods: {
    ...
    calcWidths() {
      ...
      const allWidth = avgWidth * columnNum;
      for(let i=0;i<columnNum;i++){
        if(i===0){
          list.push(allWidth*0.4);
        }else{
          list.push(allWidth*0.15);
        }
      }
      const widths = list;//new Array(columnNum).fill(avgWidth)
    ...
    async animation(start = false) {
      ...
      if (updater !== this.updater || this.isPaused) return
