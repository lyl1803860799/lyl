      ...
      <a-collapse v-model="activeKey" expandIconPosition="right">
        <a-collapse-panel key="1">
          <template slot="header">
            指标设置
            <span style="font-size: 12px;color:#FFA85A;padding-left:6px;">（最多支持20个指标）</span>
          </template>
...
import {
  queryIndexInfo,
  queryIndexData,
  queryRealTimeIndexInfo,
  queryRealTimeIndexData
} from "@/api/template";
...
export default {
  ...
  methods: {
    ...
    fetchIndexInfo() {
      if (this.children.length && this.children.length > 19) {
        this.$message.warning("最多添加20个指标");
        return;
      }
      ...
      this.data = [];
      this.fetching = true;
      if (this.title.name === "实时行情") {
        queryRealTimeIndexInfo(value).then(res => {
          console.log("实时行情", res);
            ...
            if (res.success) {
              ...
              if (res.data && res.data.length) {
                ...
                resDatas.map((item, index) => {
                  ...
                  dataList.push({
                    名称: val[index]["otherName"]
                      ? val[index]["otherName"]
                      : item.indexName,
                    最新值: lasterPrice,
                    涨跌:
                      priceRise === "-" ? null : Number(priceRise).toFixed(2),
                    涨跌幅:
                      priceRisePercent === "-"
                        ? null
                  ...
                  codeList.push({
                    ...
                    otherName: val[index]["otherName"]
                      ? val[index]["otherName"]
                      : null
                  });
                });
                this.indexCode = codeList;
              }
                  ...
                  dataList.push({
                    ...
                    涨跌幅:
                      priceRisePercent === "-"
                        ? null
                        : Number(priceRisePercent).toFixed(2),
                    发布日期: item.dataDate ? item.dataDate : "-"
                  });
                  codeList.push({
                    key: item.indexCode,
                    label: item.indexName,
                    otherName: val[index]["otherName"]
                      ? val[index]["otherName"]
                      : null,
                    frequencyCode: val[index]["frequencyCode"]
                  });
                });
                this.indexCode = codeList;
              }
    ...
    handleChange(indexCode) {
      if (indexCode.length && indexCode.length > 19) {
        this.$message.warning("最多添加20个指标");
        return;
      }
      let list = this.deepClone(this.children);
